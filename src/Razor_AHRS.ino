...
/*
  ...
  "#o<params>" - Set OUTPUT mode and parameters. The available options are:
	  ...
	  "#oh" - Output angles HAT frame format  (HAT Modification)
      "#oZ" - Save Zero offset YPR in EEPROM  (HAT Modification)
      "#oz" - Read saved zero offset YPR from EEPROM (HAT Modification)
...
#define OUTPUT__FORMAT_HAT 2    // Outputs data as binary HAT format

// Select your startup output mode and format here!
int output_mode = OUTPUT__MODE_ANGLES;
int output_format = OUTPUT__FORMAT_HAT;
...
#define ACCEL_Y_MIN ((float) -271)
#define ACCEL_Y_MAX ((float) 237)
#define ACCEL_Z_MIN ((float) -274)
...
#define GYRO_AVERAGE_OFFSET_X ((float) 1.90)
#define GYRO_AVERAGE_OFFSET_Y ((float) 22.13)
#define GYRO_AVERAGE_OFFSET_Z ((float) -2.91)
...
// ************************ HAT modification for FaceTRackNoIR *************************************
// HAT structure
typedef struct  {
...
#define RAZOR_EEPROM_BASE 0xA0
#define RAZOR_EEPROM_SIGNATURE 0x55
...
// ================================================================
void RazHatOffset() {
  eprom_save.rot_offset[0] = 0;
  eprom_save.rot_offset[1] = 0;
  eprom_save.rot_offset[2] = 0;
  eprom_save.trans_offset[0] = 0;
  eprom_save.trans_offset[1] = 0;
  eprom_save.trans_offset[2] = 0;
  SaveHatOffsets();
}
...
// ================================================================
void SaveHatOffsets() {
  eprom_save.sig_HAT=RAZOR_EEPROM_SIGNATURE;
  eeprom_write_block((const void*)&eprom_save, (void*) RAZOR_EEPROM_BASE, sizeof(eprom_save));
}
...
// ================================================================
void ReadHatOffsets() {
   eeprom_read_block( (void*)&eprom_save, (void*) RAZOR_EEPROM_BASE, sizeof(eprom_save));
    if (eprom_save.sig_HAT!=RAZOR_EEPROM_SIGNATURE) {
      RazHatOffset();
    }
}
...
// ================================================================
void PrintHatMsg(uint16_t code,char Msg[24] ) {
  msginfo.Code=code;
...
void setup()
{
  ...
  if ( output_format == OUTPUT__FORMAT_HAT) {
    PrintHatMsg(5000,"HAT SETUP");
  }
  ...
  if (Serial.available() >= 2)
  {
    if (Serial.read() == '#') // Start of new control message
    {
      ...
      else if (command == 'o') // Set _o_utput mode
      {
		...
		else if (output_param == 'h') // Output angles in _h_at format
        {
          output_mode = OUTPUT__MODE_ANGLES;
          output_format = OUTPUT__FORMAT_HAT;
          hat_center_ask=false;
          hat_center_cpt=9999;
          PrintHatMsg(5001,"HAT START");
        }
        else if (output_param == 'Z') { // Save _Z_ero offset YPR in EEPROM
          RazHatOffset();
          hat_center_ask=true;
          hat_center_cpt=0;
          PrintHatMsg(5002,"HAT SAVE CENTER");
        }
        else if (output_param == 'z') { // Read saved z_ero offset YPR from EEPROM
          ReadHatOffsets();
          hat_center_ask=false;
          hat_center_cpt=9999;
          PrintHatMsg(5003,"HAT READ CENTER");
        }
